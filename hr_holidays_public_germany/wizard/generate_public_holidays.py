...
class GeneratePublicHolidays(models.TransientModel):
    _name = 'generate.public.holidays'
    year = fields.Integer('Year', required=True, default=(lambda self: datetime.today().year))
    country_id = fields.Many2one('res.country', string='Country', required=True, domain=[('code','in',COUNTRIES)])
...
#            self.state_id = self.template_id.state_id and self.template_id.state_id.id or False
    ...
    def action_generate_de_holidays(self):
        ...
        for wizard in self:
            existing_holidays = public_holiday_obj.search([('year', '=', self.year)])
            if not existing_holidays:
                existing_holidays = public_holiday_obj.create({'year': wizard.year, 'country_id': wizard.country_id.id})
            ...
            wizard.calculate_state_floating_holidays(existing_holidays, state=wizard.state_id)
            wizard.calculate_state_fixed_holidays(existing_holidays, state=wizard.state_id)
    ...
    def action_copy_de_holidays(self):
        ...
        for wizard in self:
            ...
            existing_holidays_year_to = public_holiday_obj.search([('year', '=', wizard.year)])
            wizard.action_delete_holidays(existing_holidays_year_to)

            # create new year for holidays
            new_holiday_year = public_holiday_obj.create({'year': wizard.year, 'country_id': wizard.country_id.id})
            ...
            for holiday in wizard.template_id.line_ids:
                ...
                new_holiday_date = "%s-%s-%s" % (wizard.year, holiday_date.month, holiday_date.day)
            ...
            wizard.calculate_state_floating_holidays(new_holiday_year, state=wizard.state_id)
    ...
    def action_run(self):
        self.ensure_one()
        if self.template_id:
            getattr(self, 'action_copy_%s_holidays' %self.country_id.code.lower(), 'action_copy_de_holidays')()
        else:
            getattr(self, 'action_generate_%s_holidays' %self.country_id.code.lower(), 'action_generate_de_holidays')()
